...
def main():
    ...
    for expert in all_experts:
        ...
        if detail:
            # 更新详情字段
            expert_data.update({
                '等级': detail.get('gradeName', ''),
                '彩种等级': detail.get('skills', ''),
                '专家简介': detail.get('explains', ''),
                '最好成绩': detail.get('bestRecord', ''),
                '粉丝数': detail.get('fans', ''),
                '彩龄': detail.get('age', ''),
                '文章数量': detail.get('articles', ''),
                '大乐透一等奖': detail.get('dltOne', ''),
                '大乐透二等奖': detail.get('dltTwo', ''),
                '大乐透三等奖': detail.get('dltThree', ''),
                '大乐透总分': detail.get('dltScore', '')
            })
    ...
    # 优化列顺序
    columns_order = [
        '专家编号', '榜单排名', '专家名', '等级', '成功率', '成绩',
        '最好成绩', '大乐透一等奖', '大乐透二等奖', '大乐透三等奖', '大乐透总分',
        '粉丝数', '彩龄', '文章数量', '彩种等级', '专家简介',
        '最新文章', 'bbxId', '名次升降'
    ]
