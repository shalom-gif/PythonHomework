...
if __name__ == "__main__":
    ...
    if data:
        ...
        excel_filename = "近100期大乐透开奖数据和中奖情况.xlsx"
        ...
        for i in range(min(3, len(df))):
            issue = df.iloc[i]
            print(f"\n期号: {issue['期号']}")
            print(f"开奖日期: {issue['开奖日期']} ({issue['星期']})")
            print(f"详情页: {issue['详情页链接']}")
            print(f"前区: {issue['中奖号码_前区']} | 后区: {issue['中奖号码_后区']}")
            print(f"销售额: {issue['总销售额(元)']:,d}元 | 奖池: {issue['奖池奖金(元)']:,d}元")

            # 显示一等奖详情
            print("\n一等奖详情:")
            print(
                f"  基本: {issue['1等奖_基本注数']}注, 单注奖金: {issue['1等奖_基本单注奖金(元)']:,d}元, 总奖金: {issue['1等奖_基本总奖金(元)']:,d}元")
            print(
                f"  追加: {issue['1等奖_追加注数']}注, 单注奖金: {issue['1等奖_追加单注奖金(元)']:,d}元, 总奖金: {issue['1等奖_追加总奖金(元)']:,d}元")
            if '1等奖_派奖注数' in issue:  # 确保有派奖字段（1-2等奖才有）
                print(
                    f"  派奖: {issue['1等奖_派奖注数']}注, 单注金额: {issue['1等奖_派奖单注金额(元)']:,d}元, 总金额: {issue['1等奖_派奖总金额(元)']:,d}元")
                print(
                    f"  追加派奖: {issue['1等奖_追加派奖注数']}注, 单注金额: {issue['1等奖_追加派奖单注金额(元)']:,d}元, 总金额: {issue['1等奖_追加派奖总金额(元)']:,d}元")

            # 显示二等奖详情
            print("\n二等奖详情:")
            print(
                f"  基本: {issue['2等奖_基本注数']}注, 单注奖金: {issue['2等奖_基本单注奖金(元)']:,d}元, 总奖金: {issue['2等奖_基本总奖金(元)']:,d}元")
            print(
                f"  追加: {issue['2等奖_追加注数']}注, 单注奖金: {issue['2等奖_追加单注奖金(元)']:,d}元, 总奖金: {issue['2等奖_追加总奖金(元)']:,d}元")
            if '2等奖_派奖注数' in issue:  # 确保有派奖字段（1-2等奖才有）
                print(
                    f"  派奖: {issue['2等奖_派奖注数']}注, 单注金额: {issue['2等奖_派奖单注金额(元)']:,d}元, 总金额: {issue['2等奖_派奖总金额(元)']:,d}元")
                print(
                    f"  追加派奖: {issue['2等奖_追加派奖注数']}注, 单注金额: {issue['2等奖_追加派奖单注金额(元)']:,d}元, 总金额: {issue['2等奖_追加派奖总金额(元)']:,d}元")